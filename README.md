...
| **BD MP Mouth Parts** | Separate an **isolated mouth render** into **lips / teeth / tongue** by colour (no model) and pack them **RGBA for the Unreal viseme atlas: R=lips, G=teeth, B=tongue, A=POM** (wire depth into `pom`). Tuned-HSV engine: teeth = bright/desaturated, tongue = warm hue region-grown + interior-gated (no false-positive on warm lip corners), lips = saturated remainder (cavity excluded). Optional `edge_refine`. Replaces the `SAM3×3 → FillHoles → PackChannels` chain in the lip-viseme atlas. |
