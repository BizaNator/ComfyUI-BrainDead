...
| **BD Parts Builder 2 Plates (Qwen 2.1)** | Featureless plates: bald (RGBA, its alpha = the matte) -> mouthless and eyeless, each one edit of the bald, cut onto white and frame-proven (a failed stage stops the chain). Small heads re-framed; `work_size` 1024 or 2048. Also emits the eye / brow / mouth `socket_mask` and an RGB `feature_mask`, found by MediaPipe on the bald plate, for FaceMaker. |
...
| **BD Even Light** | Evens a head's lighting and keeps its facets: the light (a heavy mask-normalised blur of the luminance) is scaled by `keep_light`, the facets by `facet_gain`. It goes after a Qwen 2.1 shading pass, which reads cel-shading prompts as one hard key light. |
| **BD Compare Images** | Red-zone difference of a candidate against a reference (x gain, region tinted teal), a labelled reference / candidate / difference panel, and a score: mean difference and share of the region over a threshold. |
