"""
BrainDead Qwen Image Tokenizer
Created by BizaNator for BrainDeadGuild.com
A Biloxi Studios Inc. Production

Custom tokenizer for Qwen-Image models with character consistency templates.
"""
...
class Qwen25_7BVLITokenizer(sd1_clip.SDTokenizer):
    def __init__(self, embedding_directory=None, tokenizer_data={}):
        ...
        super().__init__(tokenizer_path, pad_with_end=False, embedding_size=3584, embedding_key='qwen25_7b',
                        tokenizer_class=Qwen2Tokenizer, has_start_token=False, has_end_token=False,
                        pad_to_max_length=False, max_length=99999999, min_length=1, pad_token=151643,
...
class QwenImageTokenizer(sd1_clip.SD1Tokenizer):
    def __init__(self, embedding_directory=None, tokenizer_data={}):
        ...
        self.image_roles = None

        # Enhanced English system prompt for character consistency
...
"""
    ...
    def tokenize_with_weights(self, text, return_word_ids=False, llama_template=None, images=[], disable_weights=False, **kwargs):
        if llama_template is None:
            ...
            if "Picture" in text:
                llama_text = llama_template.format(text.split("Picture")[-1].split(":")[-1].strip() if "{}" in llama_template else "")
                if "{}" not in llama_template:
                    llama_text = llama_template.replace("<|im_end|>", text + "<|im_end|>", 1)
            else:
        ...
        for r in qwen_tokens:
            for i in range(len(r)):
                if r[i][0] == 151655:  # Image token ID
                    if len(images) > embed_count:
                        ...
                        if self.image_weights and embed_count < len(self.image_weights):
                            image_data["weight"] = self.image_weights[embed_count]
                        if self.image_roles and embed_count < len(self.image_roles):
                            image_data["role"] = self.image_roles[embed_count]

                        r[i] = (image_data,) + r[i][1:]
                        embed_count += 1
...
class Qwen25_7BVLIModel(sd1_clip.SDClipModel):
    def __init__(self, device="cpu", layer="last", layer_idx=None, dtype=None, attention_mask=True, model_options={}):
        super().__init__(device=device, layer=layer, layer_idx=layer_idx, textmodel_json_config={}, dtype=dtype,
                        ...
                        model_class=comfy.text_encoders.llama.Qwen25_7BVLI, enable_attention_masks=attention_mask,
