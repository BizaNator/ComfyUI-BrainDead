...
| `BD-parts_builder_2` | Qwen Image 2.1 head layers, prompts only (no SAM). Load a MASTER 1 headshot (alpha = head mask) → BD Parts Vocabulary → QwenVL closed-list presence vote + Lotus-2 depth → **BD Parts Builder 2** (visible + complete layers, head without items) → **BD Parts Builder 2 Plates** (bald → mouthless / eyeless, frame-proven) → **BD Parts Builder 2 Assemble** (engine stack vs the source) → two BD Parts Export: visible layers + head without items (a PSD that rebuilds the source) and `engine_parts` (the engine-stack PSD). |
